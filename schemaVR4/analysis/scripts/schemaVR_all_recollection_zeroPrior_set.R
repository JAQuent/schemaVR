...
# */
# Model 3
model_schemaVR4_rem_set1 <- brm(remembered ~ sExp +  I(sExp*sExp) + set +
                             (1 | objNum) +
                             (1 | subNum),
...
# Beep when ready 
Sys.sleep(waitTime)
summary(model_schemaVR4_rem_set1)
beep(8)

# Model 3
model_schemaVR_all_rem_set2 <- brm(remembered ~ sExp*set +  I(sExp*sExp)*set +
                             (1 | objNum) +
                             (1 | subNum),
                           data = combinedData_remember,
                           prior = prior_zero,
...
# Beep when ready 
Sys.sleep(waitTime)
summary(model_schemaVR_all_rem_set2)
beep(8)


# Model 3
model_schemaVR_all_rem_no_set <- brm(remembered ~ sExp +  I(sExp*sExp) +
                                   (1 | objNum) +
                                   (1 | subNum),
                           data = combinedData_remember,
                           prior = prior_zero,
...
# Beep when ready 
Sys.sleep(waitTime)
summary(model_schemaVR_all_rem_no_set)
beep(8)

# /* 
# ----------------------------- Saving image ---------------------------
# */
save.image(datedFileNam('schemaVR_all_recollection_zeroPrior_set', '.RData'))