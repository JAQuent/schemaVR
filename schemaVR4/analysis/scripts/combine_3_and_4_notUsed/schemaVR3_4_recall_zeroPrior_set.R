# Script to run analysis of recall accuracy data for schemaVR3 & 4 with set as covariate (zero prior version)
# Version 1.0
# Date:  24/07/2021
...
# ----------------------------- Model ---------------------------
# */
model_schemaVR3_4_recall_set <- brm(accRecall ~ sExp*set +  
                                I(sExp*sExp)*set +
                                (1 | subNum) +
                                (1 | objNum),
...
# Beep 
summary(model_schemaVR3_4_recall_set)
beep(8)

# /* 
# ----------------------------- Saving image ---------------------------
# */
save.image(datedFileNam('schemaVR3_4_recall_zeroPrior_set', '.RData'))
