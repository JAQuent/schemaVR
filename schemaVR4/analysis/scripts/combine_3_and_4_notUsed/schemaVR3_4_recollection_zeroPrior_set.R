# Script to run analysis of recollection data for schemaVR3 & 4 with set as covariate (zero prior version)
# Version 1.0
# Date:  24/07/2021
...
# ----------------------------- Model ---------------------------
# */
model_schemaVR4_rem_set <- brm(remembered ~ sExp*set +  I(sExp*sExp)*set +
                                   (1 | objNum) +
                                   (1 | subNum),
...
remembered ~ sExp +  I(sExp*sExp) + set
remembered ~ sExp +  I(sExp*sExp)


# Beep when ready 
summary(model_schemaVR4_rem_set)
beep(8)

# /* 
# ----------------------------- Saving image ---------------------------
# */
save.image(datedFileNam('schemaVR3_4_recollection_zeroPrior_set', '.RData'))