...
expectancy_all <- combinedData_remember$objLocTargetRating
expectancy_all <- expectancy_all/100

# Get mean and SD of linear and quadratic term
# Linear
rem_mean_linear <- mean(expectancy_all)
rem_sd_linear   <- sd(expectancy_all)

# Quadratic
rem_mean_quadratic <- mean(expectancy_all^2)
rem_sd_quadratic   <- sd(expectancy_all^2)

# Cubic 
rem_mean_cubic <- mean(expectancy_all^3)
rem_sd_cubic   <- sd(expectancy_all^3)

# These values are then used scale on Gelman et al. (2008) and 
# https://github.com/stan-dev/stan/wiki/Prior-Choice-Recommendations
# Mean = 0 and SD = 0.5
combinedData_remember$Exp <- expectancy_all
...
# ----------------------------- Cubic Model ---------------------------
# */
model_schemaVRall_rem_cubic <- brm(remembered ~ I((Exp - 0.002101487)/(0.6848884*2)) + I((Exp^2 - 0.468863)/(0.3774832*2)) +
                                      #I((Exp^3 - -0.01127557)/(0.5612089*2)) +
                                      (1 | subNum) +
                                      (1 | objNum),
...
fileName   <- datedFileNam('schemaVR_all_recollection_zeroPrior_cubic', '.RData')
