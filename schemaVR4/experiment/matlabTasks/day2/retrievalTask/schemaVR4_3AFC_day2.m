function [ ] = schemaVR4_3AFC_day2(subNo, setNum)
%retrievalTask1 
% % % % % % % % % % % % % % % % % % % % % % % % % 
% 3AFC recogntion location task for schemaVR4
...
%% Setting everuthing up
    ...
    % Output files
    datafilename = strcat('data/3AFC_day2_',num2str(subNo),'.dat'); % name of data file to write to
    mSave        = strcat('data/3AFC_day2_',num2str(subNo),'.mat'); % name of another data file to write to (in .mat format)
    mSaveALL     = strcat('data/3AFC_day2_',num2str(subNo),'all.mat'); % name of another data file to write to (in .mat format)
    ...
    % Creating trials
    [objectNumber, foil1Rank, encodingRank, foil2Rank, foil1Location, encodingLocation, foil2Location] = textread(strcat('schemaVR4_3AFC_inputFile_', num2str(setNum), '.txt'),'%n %n %n %n %n %n %n', 'delimiter','\t');
    ...
    % Locations
    for i = 1:nTrial
        encodingLocationImages{i}   = imresize(imread(strcat('stimuli/', num2str(objectNumber(i)),'_' , num2str(encodingLocation(i)), '.png')), imageLocScale);
        encodingLocationTextures(i) = Screen('MakeTexture', myScreen, encodingLocationImages{i});
        
        foil1Images{i}              = imresize(imread(strcat('stimuli/', num2str(objectNumber(i)),'_' , num2str(foil1Location(i)), '.png')), imageLocScale);
        foil1Textures(i)            = Screen('MakeTexture', myScreen, foil1Images{i});
        
        foil2Images{i}              = imresize(imread(strcat('stimuli/', num2str(objectNumber(i)),'_' , num2str(foil2Location(i)), '.png')), imageLocScale);
...
%% Experimental loop
    for trial = 1:nTrial
        ...
        %% Saving data
        fprintf(datafilepointer,'%i %i %i %i %i %i %i %f %i %f %i %f %f %f %i %i %f %f %i %i %i %i\n', ...
            subNo,...
            setNum,...
            date,...
            time,...
        ...
        results{trial, 10}  = foil1Rank(trial);
        results{trial, 11} = foil2Location(trial);
        results{trial, 12} = foil2Rank(trial);
        results{trial, 13} = AFCPreTime;
        results{trial, 14} = RT(trial, 1);
        results{trial, 15} = responses(trial, 1);
        results{trial, 16} = correctness(trial, 1);
        results{trial, 17} = conPreTime;
        results{trial, 18} = RT(trial, 2);
        results{trial, 19} = responses(trial, 2);
        results{trial, 20} = left;
        results{trial, 21} = middle;
        results{trial, 22} = right;
    end
    %% End of experiment
