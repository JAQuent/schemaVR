# Script runs a power analysis for AFC  without experiment as an random effect
# Version 1.1
# Date: 15/09/2019
...
# Functions
genDat_logistic_AFC <- function(numObj, numSub, gamma00, beta1, beta2, w1_SD, w2_SD, u01_SD, u02_SD){
...
# ----------------------------- Load data ---------------------------
# */
load('bayesianMM_AFC_20190915_171906.RData')
...
sim_AFC_data  <- genDat_logistic_AFC(numObj, numSub, gamma00, beta1, beta2, w1_SD, w2_SD, u01_SD, u02_SD)
sim_AFC_model <- brm(accAFC ~ objLocTargetRating +
...
postDists[1, ] <- posterior_samples(sim_AFC_model)$b_IobjLocTargetRatingMUobjLocTargetRating

# Calculating Bayes Factors
# BRMS method: beta = 0 vs. beta != 0
bfs1.1[1]     <- hypothesis(sim_AFC_model, "IobjLocTargetRatingMUobjLocTargetRating = 0")$hypothesis$Evid.Ratio
...
areaPrior     <- integrate(dnorm, mean = combinedData_fixef[3, 1], sd = combinedData_fixef[3, 2], lower = 0, upper = Inf, abs.tol = 0)$value 
...
for(i in 2:nSim){
  sim_AFC_data  <- genDat_logistic_AFC(numObj, numSub, gamma00, beta1, beta2, w1_SD, w2_SD, u01_SD, u02_SD)
  sim_AFC_model <- update(sim_AFC_model,
                             newdata = sim_AFC_data,
                             recompile = FALSE,
                             cores = cores2use,
                             save_all_pars = TRUE,
                             sample_prior = TRUE,
  ...
  # Extracting posterior distribution
  postDists[i, ] <- posterior_samples(sim_AFC_model)$b_IobjLocTargetRatingMUobjLocTargetRating
  
  # Calculating Bayes Factors
  # BRMS method: beta = 0 vs. beta != 0
  bfs1.1[i]     <- hypothesis(sim_AFC_model, "IobjLocTargetRatingMUobjLocTargetRating = 0")$hypothesis$Evid.Ratio
...
# ----------------------------- Saving image ---------------------------
# */
save.image(datedFileNam('powerAnalysis_AFC', '.RData'))
