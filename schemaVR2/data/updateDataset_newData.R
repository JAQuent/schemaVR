...
dataLocationRecall$objName[which(dataLocationRecall$objName == 'towel')] <- 'towels' 
...
dataLocationRecall$xPositionCorrect <- NA_real_
dataLocationRecall$yPositionCorrect <- NA_real_
dataLocationRecall$zPositionCorrect <- NA_real_
...
generalExpectancyFactor                                        <- rep('expected', dim(dataLocationRecall)[1])
...
dataAFC$right      <- factor(dataAFC$right, labels = c('target', 'foil1', 'foil2')) # this was wrong
...
newCombData <- data.frame(subNum                  = rep(demographics$subNum, each = 20),
                          ...
                          recallMemory            = ddply(dataLocationRecall, c('subNum', 'objNum'), summarise, memory = memory)$memory,
                          ...
                          generalExpectancyFactor = as.factor(generalExpectancyFactor),
...
newCombData$accRecall  <- NA
newCombData$closestLoc <- NA
newCombData$next2Loc   <- NA
newCombData$next3Loc   <- NA
newCombData$next4Loc   <- NA
newCombData$next5Loc   <- NA
newCombData$next6Loc   <- NA
newCombData$next7Loc   <- NA
newCombData$next8Loc   <- NA
newCombData$next9Loc   <- NA
newCombData$next10Loc  <- NA
newCombData$next11Loc  <- NA
newCombData$next12Loc  <- NA
newCombData$next13Loc  <- NA
newCombData$next14Loc  <- NA
newCombData$next15Loc  <- NA
newCombData$next16Loc  <- NA
newCombData$next17Loc  <- NA
newCombData$next18Loc  <- NA
newCombData$next19Loc  <- NA
newCombData$next20Loc  <- NA
...
newCombData$objLocTargetNorm <- NA
newCombData$objLocFoil1Norm  <- NA
newCombData$objLocFoil2Norm  <- NA
for(i in 1:dim(newCombData)[1]){
  newCombData$objLocTargetNorm[i] <- newCombData[i, paste("loc", newCombData$targetLocation[i], sep = "")]
  newCombData$objLocFoil1Norm[i]  <- newCombData[i, paste("loc", newCombData$foil1Location[i], sep = "")]
  newCombData$objLocFoil2Norm[i]  <- newCombData[i, paste("loc", newCombData$foil2Location[i], sep = "")]
}
...
# answerTime              Time (in seconds) the participant needed to give answer (i.e. dropping the object) from the time the object is picked up. 
...
# objLocFoil1Rating       Object/location expectancy rating (unexpected/-100 to expected/100) by the participant for object in foil 1 location.
# objLocFoil2Rating       Object/location expectancy rating (unexpected/-100 to expected/100) by the participant for object in foil 2 location.
# objLocTargetNorm        Object/location expectancy rating (unexpected/-100 to expected/100) from normative dataset for object in target location.
# objLocFoil1Norm         Object/location expectancy rating (unexpected/-100 to expected/100) from normative dataset for object in foil 1 location.
# objLocFoil2Norm         Object/location expectancy rating (unexpected/-100 to expected/100) from normative dataset for object in foil 2 location.
