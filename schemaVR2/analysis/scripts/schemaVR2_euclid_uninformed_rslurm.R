# Script to run analysis of Euclidean data for schemaVR2
# Version 1.0
# Date:  18/11/2019
...
prior_schemaVR2  <- c(prior(normal(0, 1) , class = "Intercept"),
...
model2_fun <- function(i, seed){
  if(i == 1){
    return(list(i = i,
                seed = seed,
                model = model_schemaVR2_euclid))
  } else {
    model_schemaVR2_euclid <- update(model_schemaVR2_euclid,
...
# Create rslurm job 
sjob1 <- slurm_apply(model2_fun, pars, jobname = 'schemaVR2_euclid_uniformed',
                     add_objects = c("model_schemaVR2_euclid", 'dataSchemaVR2_recall', 'prior_schemaVR2', 'iterPerChain'),
