...
# Participant FTSIQ0: Slightly different instructions for no-memory trial -> exclude.
...
# Participant ENEPJG: No memory trials on trial 1 and 2 because I pressed those accidentally -> changed.
dataSchemaVR1[which(dataSchemaVR1$subNum == 'ENEPJG' & (dataSchemaVR1$recallTrial == 1 | dataSchemaVR1$recallTrial == 2)), 'recallNoMemory'] <- 1
# Participant RZ0GTE: Problems with data and from towel -> no abnormalities.
...
# AWQ44N has NA values for recallNoMemory 
