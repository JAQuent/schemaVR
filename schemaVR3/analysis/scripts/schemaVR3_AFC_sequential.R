# Script to run analysis of AFC accuracy data for schemaVR3 (sequential)
# Version 1.0
# Date:  20/01/2022
...
targetPattern <- "schemaVR2_AFC_sequential"
...
# sleep sleepTime sec
Sys.sleep(sleepTime)
linear_schemaVR2_AFC <- brm(b_IExpM0.0854629D0.703843MU2 ~ 1,
                            data = postDists,
                            cores = cores2use,
                            family = student(link = "identity", link_sigma = "log", link_nu = "logm1"))
...
# sleep sleepTime sec
Sys.sleep(sleepTime)
quad_schemaVR2_AFC <- brm(b_IExpE2M0.5008776D0.3671161MU2 ~ 1,
                                   data = postDists,
                                   cores = cores2use,
                                   family = student(link = "identity", link_sigma = "log", link_nu = "logm1"))
...
prior_schemaVR3_AFC  <- c(set_prior(priorString_student(intercept_schemaVR2_AFC), 
                                      class = "Intercept"),
                            set_prior(priorString_student(linear_schemaVR2_AFC), 
                                      class = "b", 
                                      coef = "IExpM0.0854629D0.703843MU2"),
                            set_prior(priorString_student(quad_schemaVR2_AFC), 
                                      class = "b", 
...
model_schemaVR3_AFC <- brm(accAFC ~ I((Exp - 0.0854629)/(0.703843*2)) + I((Exp^2 - 0.5008776)/(0.3671161*2)) +
                                (1 | subNum) +
                                (1 | objNum),
...
fileName   <- datedFileNam('schemaVR3_AFC_sequential', '.RData')
