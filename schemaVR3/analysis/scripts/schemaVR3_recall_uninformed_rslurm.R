...
model2_fun <- function(i, seed){
  if(i == 1){
    return(list(i = i,
                seed = seed,
                model = model_schemaVR3_recall))
  } else {
    model_schemaVR3_recall <- update(model_schemaVR3_recall,
...
# Create rslurm job 
sjob1 <- slurm_apply(model2_fun, pars, jobname = 'schemaVR3_recall_uniformed',
                     add_objects = c("model_schemaVR3_recall", 'dataSchemaVR3_recall', 'prior_schemaVR3', 'iterPerChain'),
