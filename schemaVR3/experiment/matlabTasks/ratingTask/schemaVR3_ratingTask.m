function [] = schemaVR3_ratingTask(instructionFile, questionsFile, subNo, namePrefix)
% % % % % % % % % % % % % % % % % % % % % % % % % 
...
%% Setting everuthing up
    ...
    % Input variables
    [fileNames, questions] = textread(questionsFile, '%20s %100s', 'delimiter', '\t');
    ...
    locations = [];
    for i = 1:nTrial
        if length(fileNames{i}) > 2
            splitStr     = strsplit(fileNames{i}, '_');
            objects(i)   = str2double(splitStr{1}); % Object
            locations(i) = str2double(splitStr{2}); % Location
        else
            objects(i)   = str2double(fileNames{i}); % Object
            locations(i) = 0; % Location
        end
    end
    
    % Output variables & files
    filePointer = fopen(strcat('data/', namePrefix, '_' , num2str(subNo),'.dat'),'wt'); % opens ASCII file for writing
    mSave       = strcat('data/', namePrefix, '_', num2str(subNo),'.mat'); % name of another data file to write to (in .mat format)
    mSaveALL    = strcat('data/', namePrefix, '_', num2str(subNo),'all.mat'); % name of another data file to write to (in .mat format)
