...
normativeData <- read.csv.csv('ratingsForSets.csv')
...
for(i in 1:nSims){
 ...
 for(j in sets){
   ...
   meanExpectancyTemp         <- normativeData[normativeData$set == j, 'expectancy'] + rnorm(nObj, 0, expSD)
