...
priors_6  <- c(prior(student_t(6, 0, 10) , class = "Intercept"),
               prior(student_t(6, 0, 1) , class = "b")) 

priors_7  <- c(prior(student_t(7, 0, 10) , class = "Intercept"),
               prior(student_t(7, 0, 1) , class = "b")) 

priorDensity_3 <- dstudent_t(0, 3, 0, 1)
priorDensity_4 <- dstudent_t(0, 4, 0, 1)
priorDensity_5 <- dstudent_t(0, 5, 0, 1)
priorDensity_6 <- dstudent_t(0, 6, 0, 1)
priorDensity_7 <- dstudent_t(0, 7, 0, 1)
...
# ----------------------------- Simulation function --------------------------
# */
sim_function <- function(index, seed, numSub, beta0, beta1, priorCond){
  #  Generate data
  df <- data_generator(numSub, beta0, beta1)
  
  # Get model
  baseModel <- get(paste0('baseModel_', priorCond))
  ...
  # Calculate BF if no error  
    if(!error){
      ...
      # Get prior density
      priorDensity <- get(paste0('priorDensity_', priorCond))
...
params <- data.frame(index = 1:totalIter,
                     ...
                     priorCond = rep(prior, nIter))

# /*
# ----------------------------- Submitting job --------------------------
# */
jobName <- "logistic_regression2"
