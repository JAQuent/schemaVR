# Simulation for priors in logistic regression (Extra with scale 0.5)
# Date: 12/11/2020
...
# Student prior
# 0.5
priors_student_0.5 <- c(prior(student_t(7, 0, 10) , class = "Intercept"),
               prior(student_t(7, 0, 0.5) , class = "b")) 
priorDensity_student_0.5 <- dstudent_t(0, 7, 0, 0.5)

# Normal
# 0.5
priors_normal_0.5  <- c(prior(normal(0, 10) , class = "Intercept"),
                        prior(normal(0, 0.5) , class = "b")) 
priorDensity_normal_0.5 <- dnorm(0, 0, 0.5)
...
baseModel_student_0.5 <- brm(y ~ s_x,
                   data = df,
                   prior = priors_student_0.5,
                   family = bernoulli(),
                   chains = 1,
                   save_all_pars = TRUE,
                   sample_prior = TRUE,
...
baseModel_normal_0.5 <- brm(y ~ s_x,
                          data = df,
                          prior = priors_normal_0.5,
                          family = bernoulli(),
                          chains = 1,
                          save_all_pars = TRUE,
                          sample_prior = TRUE,
...
# ----------------------------- Submitting job --------------------------
# */
jobName <- "logistic_regression3_extra"
sjob1 <- slurm_apply(sim_function, params, jobname = jobName,
                     add_objects = c('data_generator', 'baseModel_student_0.5', 'baseModel_normal_0.5', 'iterPerChain', 'priorDensity_student_0.5', 'priorDensity_normal_0.5'),
