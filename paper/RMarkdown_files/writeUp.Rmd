...
## Discussion
This experiment confirmed the SLIMM prediction of a U-shaped function relating object-location memory to the expectancy of that location, with better memory for highly expected or highly unexpected locations . This replicates the pattern of Greve et al (under review), but using much more realistic stimuli and pre-experimental knowledge, rather than arbitrary rules learned during an experiment. It also extends the prior work of @Lew2016 in examining a range of expectancy ratings, rather than just comparing the two end-points of highly expected and highly unexpected (as is required to detect a U-shape). Furthermore, the experiment addressed the methodological problem of guessing at test, which would improve performance for highly expected locations (even if memory is not in fact superior). The presence of this guessing bias was confirmed by looking at incorrect recall trials, but it was not sufficient to overcome the additional advantage of unexpected locations that was seen in the recall accuracy and recall error. Furthermore, guessing would be unlikely to help performance on the 3AFC test, since the foils were approximately matched on expectancy, and yet the 3AFC data still showed the same numerical advantage for highly expected locations (with a quadratic component that would be significant under a one-tailed test in the predicted direction). In contrast to what we expected there was a U-shaped relation for the recall task, too. 

There was also a memory advantage of general expectancy of each object appearing in the kitchen. This was driven largely by the 8 non-kitchen objects. However, these non-kitchen objects also tended to have location expectancy values close to zero (which is to be expected, since if for example a hardhat is rarely encountered in a kitchen, then one does not have strong predictions about where in the kitchen it should be). It is possible that the quadratic fit to object-location memory was influenced by the reduced performance predicted after adjusting for the overall advantage of non-kitchen objects. In fact, the quadratic component remained significant in the above analyses even if general expectancy was removed from the model. Nonetheless, an important aim of Experiment 2 was to replicate the U-shape even when some kitchen objects were placed in locations with middling expectancy values (by re-selecting locations from the normative data), so that the quadratic fit was not overly-influenced by the overall memory advantage for non-kitchen objects.  
...
cite Wang with schema and familiarity
...
## Discussion
The main aim of Experiment 2 was to show that the U-shaped function in Experiment 1 was not an artifact of the fact that the kitchen objects were at the extremes of location expectancy, while the neutral expectancy region was occupied by non-kitchen objects. A new set of object/location combinations was created that maximised the spread of the kitchen items in terms of their object/location expectancy. The U-shape was replicated for Euclidean error and for 3AFC accuracy, and approached significance for recall accuracy.

A further aim was to test whether the two ends of the U-shape were associated with different phenomenological experiences during retrieval, specifically recollection for unexpected locations and familiarity for expected locations, as predicted by SLIMM. A categorical comparison based on expected versus unexpected locations  did not show an obvious difference, but there was some hint of this in a correlational analysis , where a negative correlation between the percentage of remember responses and the object/location expectancy of object approached significance, while the reverse was true for the percentage of know responses. These results need to be interpreted with caution because the correlations were based on just twenty objects. Future attempts should optimise the design to have more power for detecting such an effect.

Finally, unlike the first experiment, higher general expectancy ratings were not associated with worse recall (e.g,, not benefit for non-kitchen objects). The reason for this is unclear.

The effect that remember responses are generally faster than know responses were also replicated [e.g. @Rotello2008]. 
...
# Experiment 4
maybe add item lures to study incidental memory

# Acknowledgement
We thank Adina Lew to kindly share her stimuli with us. 
Buerkner
