...
```{r}
# Loading data
load(paste0(path2parent, "/schemaVR4/analysis/schemaVR4_recall_searchlight_20210516_223646.RData"))

# Get breaking points
br1  <- results$breakingPoint[1]
br6  <- results$breakingPoint[6]
br10 <- results$breakingPoint[10]

# Get slopes and BF
sl1    <- round(results$slope1[6], 2)
CI_l1  <- round(results$q2.5_1[6], 2)
CI_u1  <- round(results$q97.5_1[6], 2)
BF_sl1 <- round(results$bf_OR_1[6], 2)
sl2    <- round(results$slope2[6], 2)
CI_l2  <- round(results$q2.5_2[6], 2)
CI_u2  <- round(results$q97.5_2[6], 2)
BF_sl2 <- round(results$bf_OR_2[6], 2)
```

We tested equally spaced breaking points between `r br1` and `r br10`. As predicted we found evidence for a true u-shape at breakpoint `r br6` where we estimated slope 1 to be $\beta$ = `r sl1` (95 % CI [`r CI_l1`, `r CI_u1`]), $BF_{10}$ = `r BF_sl1` and slope 2 to be  $\beta$ = `r sl2` (95 % CI [`r CI_l2`, `r CI_u2`]), $BF_{10}$ = `r BF_sl2`, where the CI for slope 1 & 2 did not overlap with zero. The quantified evidence in terms of the $BF_{10}$ was convincing based on our simulations. 
...
path2parent <- path2parent_current
# Loading data
load(paste0(path2parent, "/schemaVR4/analysis/schemaVR4_AFC_searchlight_20210515_094333.RData"))

# Get breaking points
br1  <- results$breakingPoint[1]
br3  <- results$breakingPoint[3]
br4  <- results$breakingPoint[4]
br10 <- results$breakingPoint[10]

# Get slopes and BF
sl1    <- round(results$slope1[4], 2)
CI_l1  <- round(results$q2.5_1[4], 2)
CI_u1  <- round(results$q97.5_1[4], 2)
BF_sl1 <- round(results$bf_OR_1[4], 2)
sl2    <- round(results$slope2[4], 2)
CI_l2  <- round(results$q2.5_2[4], 2)
CI_u2  <- round(results$q97.5_2[4], 2)
BF_sl2 <- round(results$bf_OR_2[4], 2)
```

We tested equally spaced breaking points between `r br1` and `r br10`. As predicted we found evidence for a true u-shape at breakpoint `r br4` where we estimated slope 1 to be $\beta$ = `r sl1` (95 % CI [`r CI_l1`, `r CI_u1`]), $BF_{10}$ = `r BF_sl1` and slope 2 to be  $\beta$ = `r sl2` (95 % CI [`r CI_l2`, `r CI_u2`]), $BF_{10}$ = `r BF_sl2`, where the CI for slope 1 & 2 did not overlap with zero. The quantified evidence in terms of the $BF_{10}$ was convincing based on our simulations. 
...
path2parent <- path2parent_current
# Loading data
load(paste0(path2parent, "/schemaVR4/analysis/schemaVR4_remember_searchlight_20210515_104341.RData"))
